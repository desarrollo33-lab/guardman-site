...
async function main() {
  ...
  try {
    ...
    const processedLocations = locations.map(l => ({
      ...
      landmarks: parseJsonField(l.landmarks),
      stats: parseJsonField(l.stats),
      why_this_zone: l.why_this_zone || '',
    ...
    if (siteConfigData && Object.keys(siteConfigData).length > 0) {
      ...
      configData = {
        ...
        // FORCE correct values (these come from CMS but we override with correct ones)
        phone: DEFAULT_SITE_CONFIG.phone,
        phone_display: DEFAULT_SITE_CONFIG.phone_display,
